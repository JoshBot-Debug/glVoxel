...
private:
  static uint64_t ClearLowestBits(uint64_t bits, int n) {
    return (n >= 64) ? 0 : (bits & ~((1ULL << n) - 1));
  }

  static void SetWidthHeight(uint8_t a, uint8_t b, uint64_t bits,
                             uint64_t (&widthMasks)[],
                             uint64_t (&heightMasks)[], unsigned int chunkSize);

  static void
  PrepareWidthHeightMasks(const uint64_t (&bits)[], uint8_t paddingIndex, uint8_t (&padding)[],
  ...
  static void
  GreedyMesh64Axis(const glm::ivec3 &offsetPosition, const uint64_t (&bits)[],
                   uint64_t (&widthStart)[], uint64_t (&heightStart)[],
                   uint64_t (&widthEnd)[], uint64_t (&heightEnd)[],
